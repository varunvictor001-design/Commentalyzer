...
def main():
    ...
    print(
        "\n"
        "====================================================\n"
        " SIH26152 — GOVERNMENT SOCIAL MEDIA INTELLIGENCE\n"
        "===================================================="
    )
    ...
    # ------------------------------------------
    # REMOVE SPAM FROM DASHBOARD
    # ------------------------------------------

    usable_results = [
        r
        for r in results
        if not r.post.is_spam
    ]
    ...
    volume = (
        TrendAnalyzer.complaint_volume(
            negative_posts
        )
    )

    growth = (
        TrendAnalyzer.growth_percentage(
            volume
        )
    )
    ...
    save_choice = input(
        "\nSave detailed JSON results? (y/n): "
    ).strip().lower()
